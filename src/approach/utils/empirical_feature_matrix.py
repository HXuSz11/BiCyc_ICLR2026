...
class EmpiricalFeatureMatrix:
    ...
    def compute(self, model, trn_loader, heads, task_id):
        self.compute_efm(model, trn_loader, heads, task_id)
                

 
    def compute_efm(self, model, trn_loader, heads, task_id):
        ...
        with torch.no_grad():
            for images, targets in itertools.islice(trn_loader, n_samples_batches):
                ...
                gap_out = model(images.to(self.device))
                
                
                out = torch.cat([h(gap_out)for h in heads], dim=1)
                ...
                weight_matrix = torch.cat([h[0].weight for h in heads], dim=0) 
